    ...
    <div>
      ...
      <div className="breadcrumb">
        <div className="max-w-7xl mx-auto breadcrumb-text">
          ...
          {tenant.category && typeof tenant.category === 'object' && (
            <>
              {' / '}
              <Link href={`/category/${tenant.category.slug}`} className="hover:text-gold">{tenant.category.name}</Link>
            </>
          )}
          {' / '}
      ...
      <section className="max-w-7xl mx-auto py-16 px-8 grid grid-cols-1 lg:grid-cols-3 gap-16">
        ...
        <div className="lg:col-span-2">
          <div className="mb-12">
            <h2 className="text-3xl font-display mb-6 border-b border-gold/30 pb-4">The Boutique</h2>
            {tenant.description ? (
        ...
        <div className="lg:col-span-1 bg-gray-50 dark:bg-zinc-900 p-8 border border-gray-100 dark:border-zinc-800 h-fit">
          ...
          {tenant.suiteNumber && (
            <div className="mb-8">
              ...
              <a
                ...
                className="text-sm text-gold font-semibold underline hover:text-gold/80 transition-colors"
              >
          ...
          <div className="mb-8 border-t border-gray-200 dark:border-zinc-700 pt-8">
            ...
            {tenant.phone && (
              <p className="text-lg font-display mb-4">
                <a href={`tel:${tenant.phone}`} className="hover:text-gold transition-colors">{tenant.phone}</a>
              </p>
            )}
            {tenant.email && (
              <p className="text-sm mb-4">
                <a href={`mailto:${tenant.email}`} className="text-gold hover:underline">{tenant.email}</a>
              </p>
            )}
            {tenant.website && (
              <a
          ...
          <div className="space-y-3">
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400 hover:text-gold transition-colors"
            >
          ...
          {tenant.status === 'coming-soon' && (
            <div className="text-center">
              <span className="inline-block px-4 py-2 bg-gold text-white text-xs font-semibold uppercase tracking-wider">
                Coming Soon
              </span>
            </div>
          )}
          ...
          <div className="max-w-7xl mx-auto px-8">
            <div className="flex justify-between items-end mb-12">
              ...
              <div className="hidden md:flex gap-4">
                <button className="w-10 h-10 border border-gray-300 dark:border-zinc-700 flex items-center justify-center hover:bg-gold hover:border-gold hover:text-white transition-all">
                ...
                >
                  <div className="w-full aspect-square overflow-hidden bg-white dark:bg-zinc-800 border border-gray-100 dark:border-zinc-700 flex items-center justify-center p-6">
                    {related.logo && typeof related.logo === 'object' ? (
                      ...
                      <span className="text-4xl font-display text-gray-200 group-hover:text-gold transition-colors">
                        {related.name.charAt(0)}
                      </span>
                    )}
                  </div>
                  <span className="mt-4 text-xs tracking-[0.3em] uppercase font-semibold text-center group-hover:text-gold transition-colors">
                    {related.name}
                  </span>
                </Link>
              ))}
